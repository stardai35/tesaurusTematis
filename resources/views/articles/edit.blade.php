...
<div class="container mt-5 mb-5">
    <div class="row">
        <div class="col-md-10 mx-auto">
            <h1 class="mb-4">Edit Artikel: {{ $article->title }}</h1>
            ...
            <form action="{{ route('articles.update', $article) }}" method="POST" id="articleForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="number" class="form-control @error('num') is-invalid @enderror" id="num" name="num" value="{{ old('num', $article->num) }}">
                                @error('num')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('articles.show', $article) }}" class="btn btn-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
    ...
    function populateWordRelations() {
        ...
        if (relations.length === 0) {
            ...
            relations.forEach(relation => {
                ...
                // Set lemma
                const lemmaSelect = clone.querySelector('.lemma-select');
                lemmaSelect.value = relation.lemma_id;
                
                // Set wordclass
                const wordclassSelects = clone.querySelectorAll('select[name*="wordclass_id"]');
                wordclassSelects[0].value = relation.wordclass_id;
                
                // Set type
                const typeSelects = clone.querySelectorAll('select[name*="type_id"]');
                typeSelects[0].value = relation.type_id;
                
                // Set word_order
                const wordOrderInputs = clone.querySelectorAll('input[name*="word_order"]');
                wordOrderInputs[0].value = relation.word_order;
                
                // Set description
                const descriptionTextareas = clone.querySelectorAll('textarea[name*="description"]');
                descriptionTextareas[0].value = relation.description;
