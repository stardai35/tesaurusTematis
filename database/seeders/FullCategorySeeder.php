...
class FullCategorySeeder extends Seeder
{
    public function run(): void
    {
        ...
        $articleNumber = 1; // Counter untuk nomor artikel 1-522

        foreach ($data as $item) {
            // Insert category
            $categoryId = DB::table('category')->insertGetId([
                'num' => $item['category']['num'],
                'title' => $item['category']['title'],
                'slug' => Str::slug($item['category']['title']),
            ]);

            // Insert subcategories and articles
            foreach ($item['subcategories'] as $subcat) {
                ...
                // Insert articles
                foreach ($subcat['articles'] as $articleTitle) {
        ...
        $this->command->info('Full category structure with articles seeded successfully!');
    }
}
