...
@section('title', 'Tambah Relasi Kata Baru')
...
<p class="page-subtitle">Hubungkan lemma dengan artikel untuk membangun struktur tesaurus</p>
...
<form method="POST" action="{{ route('admin.word-relations.store') }}">
                ...
                <select name="lemma_id" id="lemma_id" required>
                    ...
                    @foreach($lemmas as $lemma)
                        <option value="{{ $lemma->id }}" 
                            {{ old('lemma_id') == $lemma->id ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <select name="wordclass_id" id="wordclass_id" required>
                    ...
                    @foreach($wordClasses as $wc)
                        <option value="{{ $wc->id }}" 
                            {{ old('wordclass_id') == $wc->id ? 'selected' : '' }}>
                            {{ $wc->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="relationship_type" id="relationship_type">
                    ...
                    @foreach($labelTypes as $lt)
                        <option value="{{ $lt->id }}" 
                            {{ old('relationship_type') == $lt->id ? 'selected' : '' }}>
    ...
    <div class="form-section">
        ...
        <div class="form-row form-row-3">
            <div class="form-group">
                ...
                <input type="number" name="par_num" id="par_num" min="0" value="{{ old('par_num') }}">
                <div class="form-help">Paragraf ke berapa (opsional)</div>
            </div>

            <div class="form-group">
                <label for="group_num">Nomor Grup</label>
                <input type="number" name="group_num" id="group_num" min="0" value="{{ old('group_num') }}">
                <div class="form-help">Grup ke berapa dalam paragraf (opsional)</div>
            </div>

            <div class="form-group">
                <label for="word_order">Urutan Kata</label>
                <input type="number" name="word_order" id="word_order" min="0" value="{{ old('word_order') }}">
    ...
    <div class="form-section">
        ...
        <div class="form-group">
            ...
            <input type="text" name="meaning_group" id="meaning_group" placeholder="Contoh: makna1, makna2" 
                   value="{{ old('meaning_group') }}">
            <div class="form-help">Kelompok makna yang ini kurangi</div>
        </div>

        <div class="form-group">
            <label for="description">Deskripsi / Catatan</label>
            <textarea name="description" id="description" placeholder="Penjelasan tentang relasi kata ini...">{{ old('description') }}</textarea>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" name="foreign_language" id="foreign_language" 
                       placeholder="Contoh: English equivalent" value="{{ old('foreign_language') }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="language_variant" id="language_variant" 
                       placeholder="Contoh: Bahasa Melayu, Dialek Betawi" value="{{ old('language_variant') }}">
    ...
    <div class="form-actions">
        <button type="submit" class="btn-submit">✅ Simpan Relasi Kata</button>
