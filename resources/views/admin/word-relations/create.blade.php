...
@section('page-title', 'Tambah Word Relation Baru')

@section('content')
<div class="page-header">
    <h1 class="page-title">Tambah Word Relation Baru</h1>
</div>

<div class="card">
    <form action="{{ route('admin.word-relations.store') }}" method="POST">
        @csrf
            ...
            <div class="form-group">
                ...
                <select name="wordclass_id" class="form-control" required>
                    ...
                    @foreach($wordClasses as $wc)
                        <option value="{{ $wc->id }}" @selected(old('wordclass_id') == $wc->id)>{{ $wc->name }}</option>
                    @endforeach
                </select>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div class="form-group">
                ...
                <input type="number" name="group_num" class="form-control" placeholder="Group num" value="{{ old('group_num', 1) }}" min="1">
                @error('group_num') <small style="color: #ef4444;">{{ $message }}</small> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">Meaning Group</label>
                <input type="number" name="meaning_group" class="form-control" placeholder="Meaning group" value="{{ old('meaning_group', 1) }}" min="1">
        ...
        <div class="form-group">
            ...
            <textarea name="description" class="form-control" placeholder="Masukkan deskripsi" style="resize: vertical; min-height: 100px;">{{ old('description') }}</textarea>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div class="form-group">
                ...
                <input type="text" name="foreign_language" class="form-control" placeholder="Bahasa asing" value="{{ old('foreign_language') }}">
                @error('foreign_language') <small style="color: #ef4444;">{{ $message }}</small> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">Variasi Bahasa</label>
                <input type="text" name="language_variant" class="form-control" placeholder="Variasi bahasa" value="{{ old('language_variant') }}">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div class="form-group">
                ...
                <input type="text" name="relationship_type" class="form-control" placeholder="Tipe relasi" value="{{ old('relationship_type') }}">
                @error('relationship_type') <small style="color: #ef4444;">{{ $message }}</small> @enderror
            </div>

            <div class="form-group" style="display: flex; align-items: flex-end;">
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="checkbox" name="is_superordinate" value="1" @checked(old('is_superordinate'))>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div class="form-group" style="display: flex; align-items: flex-end;">
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                    <input type="checkbox" name="is_bold" value="1" @checked(old('is_bold'))>
                    <span>Is Bold</span>
                </label>
            </div>
        </div>
        ...
        <div style="display: flex; gap: 1rem; justify-content: flex-end;">
            ...
            <button type="submit" class="btn btn-primary">Simpan Word Relation</button>
        </div>
    </form>
</div>
@endsection
