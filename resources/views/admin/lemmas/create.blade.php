...
@section('title', 'Tambah Lemma')

@section('content')
<div class="page-header">
    <h1 class="page-title">Tambah Lemma Baru</h1>
    <p class="page-subtitle">Masukkan informasi lemma/kata baru</p>
</div>

<div class="card">
    <form method="POST" action="{{ route('admin.lemmas.store') }}">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="text" name="name_tagged" class="form-control" value="{{ old('name_tagged') }}">
            @error('name_tagged')
                <small style="color: var(--danger);">{{ $message }}</small>
            @enderror
        </div>
